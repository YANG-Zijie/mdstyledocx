...
- 新增 AIMD 兼容的 `cite` / BibTeX `refs`：按标题、段落、列表及表格中的首次引用顺序编号，复用同一资料编号，支持组合引用、Word 书签与内部跳转、带链接的简洁参考资料、字面量子集严格校验，以及 preset 控制的原始顺序、上标和参考资料样式。代码示例保留原样，未引用资料排在已引用资料之后。参考资料在首个 `refs` 块处输出，标题与分页由作者控制。
- 将行内 Markdown 链接导出为原生 Word 外部超链接，支持标题、正文、列表和表格单元格。
