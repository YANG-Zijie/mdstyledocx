...
- 表头与首条数据行保持同页，避免页末仅留下表头；保留续页重复表头及后续数据行的正常分页。仅有表头的空表不会与后续内容绑定。
- 将安装示例和仓库 Skill 的版本要求同步至 0.4.2，确保导出包含本次分页修复。

## 0.4.1 - 2026-09-11

- 修复四个中文公文预设中三级标题（`####`）缺少首行缩进的问题：三号字体缩进 32 pt，小四号字体缩进 24 pt，均为两字符。
- 增加带编号与不带编号标题的 DOCX 回归测试，并补全预设规则说明。

## 0.4.0 - 2026-09-09

- 新增 AIMD 兼容的 `cite` / BibTeX `refs`，按标题、段落、列表及表格中的首次引用顺序编号，复用同一资料编号并支持组合引用。
- 引用可跳转至原生 Word 参考资料书签，资料标题可链接到 URL 或 DOI；支持由 preset 控制原始顺序、上标和参考资料样式。
- 严格校验 BibTeX 字面量子集、缺失引用目标和重复 ID。代码中的引用示例保留原样，未引用资料排在已引用资料之后；参考资料在首个 `refs` 块处输出，标题与分页由作者控制。
- 将行内 Markdown 链接导出为原生 Word 外部超链接，支持标题、正文、列表和表格单元格。
- 在连续的 Markdown 表格之间按当前 preset 的正文行高插入一个空段落，避免导出为 DOCX 后两张表格黏连。
- 将安装示例和 Skill 的版本要求同步至 0.4.0，并修正页眉页脚示例以使用受支持的左右两区页眉。
