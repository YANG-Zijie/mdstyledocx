...
- `-` / `*` / `+`：无序列表
- `1.`：有序列表
- `[文字](https://example.com)`：可点击的 Word 外部超链接
...
使用标准 Markdown 行内链接写法：

```md
- [ICH M3(R2)：支持药物临床试验和上市许可的非临床安全性研究](https://database.ich.org/sites/default/files/M3_R2__Guideline.pdf)
```

导出后只显示链接文字，目标地址写入 DOCX 的外部超链接关系，可在 Word 中点击打开。超链接可用于标题、正文、列表和表格单元格；链接文字内部暂不解析加粗、斜体等嵌套 Markdown 样式。
...
- 开发版新增：表头与首条数据行保持同页，避免页末孤立表头；后续数据行仍可正常跨页
