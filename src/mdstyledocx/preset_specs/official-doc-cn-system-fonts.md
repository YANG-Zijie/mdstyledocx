# `official-doc-cn-system-fonts` preset

## 引用与参考资料

正文用 `{{cite|id}}` 或 `{{cite|id1,id2}}`，资料定义用 BibTeX `refs` 代码块。默认按全文首次引用顺序编号并生成 Word 内部跳转；未引用条目排在最后。`citation_settings.order: source` 可保留定义顺序，`citation_settings.superscript: true` 可启用上标。资料标题通过 URL 或 DOI 链接到来源。

参考资料在首个 `refs` 块处输出一次，标题及 `<!-- pagebreak -->` 由作者安排。默认采用正文的字体、字号和行距及两字符悬挂缩进，可用完整的 `styles.reference` 样式覆盖。简洁著录不等同于完整 GB/T 7714 格式。支持范围及 BibTeX 校验规则见 README 的“引用与参考资料”部分。

## 适用场景

中文公文、正式通知、情况报告、请示类材料的通用字体名称方案。

## 结构化样式表

- 页面大小：A4
- 页边距：上/下 `2.54 cm`，左/右 `3.18 cm`（本工具的工程基线值，不宣称等同于完整公文版心规范）
- 全文文字行距：固定值 `28 磅`
- 除公文标题段后留一行外，其余文字段前、段后：`0`

| Markdown / 样式 | 字体 | 字号 | 对齐 / 字形 | 缩进 / 编号 |
| --- | --- | --- | --- | --- |
| `#` / 公文标题 | `黑体` | 二号（`22 pt`） | 居中；段后留一行 | 无 |
| `##` / 一级标题 | `黑体` | 三号（`16 pt`） | 左对齐 | 首行缩进两字符（`32 pt`）；编号按 Markdown 原文输出 |
| `###` / 二级标题 | `楷体` | 三号（`16 pt`） | 左对齐 | 首行缩进两字符（`32 pt`）；编号按 Markdown 原文输出 |
| `####` / 三级标题 | `仿宋` | 三号（`16 pt`） | 左对齐、加粗 | 首行缩进两字符（`32 pt`）；编号按 Markdown 原文输出 |
| 普通段落 / 正文 | `仿宋` | 三号（`16 pt`） | 两端对齐；标点不溢出版心 | 首行缩进两字符（`32 pt`） |
| `1.` / `-` / 列表项 | `仿宋` | 三号（`16 pt`） | 两端对齐 | 首行缩进两字符（`32 pt`）；回行顶格 |

JSON 定义中的 `paragraph_defaults` 统一控制全文文字的行距和段距，各个 `styles` 只声明字体、字号、字形、对齐和缩进等差异。图片所在段落为避免固定行距压缩图片，单独使用自适应单倍行距。

## 可选页面内容

- 页眉：仿宋，五号（`10.5 pt`），下方带 `0.5 pt` 黑色单线
- 页脚：宋体，五号（`10.5 pt`）
- 文本水印：黑体，`36 pt`，默认颜色 `#D9D9D9`、透明度 `0.25`、旋转 `-45°`

页眉、页脚和水印默认不显示。需要时在 Markdown 开头使用 YAML frontmatter：

```yaml
---
header:
  left: 某某单位
  right: 内部材料
footer:
  center: "— {page} / {pages} —"
watermark:
  text: 内部资料
---
```

页眉支持 `left`、`right`；页脚支持 `left`、`center`、`right`。两者均可使用 `{page}`、`{pages}`、`{title}`、`{date}` 动态字段。中页眉不受支持。字体、字号和水印外观由 preset JSON 控制，不写入 frontmatter。

## 推荐 Markdown 写法

- `#`：公文标题
- 标题下方直接写主送机关或称谓段落
- `##`：一级标题；需要编号时写成 `## 一、XXXX`，不需要时直接写标题内容
- `###`：二级标题；需要编号时写成 `### （一）XXXX`，不需要时直接写标题内容
- `####`：三级标题；需要编号时写成 `#### 1. XXXX`，不需要时直接写标题内容
- 普通段落：正文内容，每段空一行
- `| ... |`：原生 Word 表格；表头加粗并跨页重复，列宽根据内容分配后允许 Word 自动调整
- `![说明](./image.png)`：插入本地图片，按嵌入式输出；图片所在段落改为单倍行距
- fenced `fig`：插入自动编号并带图题、图例和可选稳定 ID 的本地图片；按“图 1：图题”输出，正文可使用 `{{ref_fig|ID}}` 引用显式 ID
- `<!-- blankline -->` / `<!-- blankline: N -->`：按正文行高空一行或指定 `1–20` 行
- `<!-- pagebreak -->`：需要分页时显式插入

标题编号属于文档内容，由 Markdown 显式写入。导出器不会自动添加、删除或重排编号，因此同一级别可以同时使用有编号和无编号标题。

`fig` / `ref_fig` 源自 [Airalogy Markdown（AIMD）的结构化图片语法](https://github.com/airalogy/airalogy/blob/main/docs/airalogy/en/syntax/fig.md)。不参与引用的图片可省略 `id`；严格的 AIMD 文件仍应显式填写。普通 `![](...)` 图片不参与编号。

## 与 `official-doc-cn` 的区别

- 不使用 `方正小标宋简体`
- 不使用 `楷体_GB2312`
- 不使用 `仿宋_GB2312`
- 更适合系统内仅提供通用 `黑体 / 楷体 / 仿宋` 字体的环境

## 示例

```md
# 关于开展示例工作的通知

各有关单位：

为统一输出格式，现将有关事项通知如下。

## 一、工作目标

### （一）总体要求

#### 1. 任务分工

请各单位提高认识，认真落实。

## 结语

请结合实际抓好后续工作。
```
